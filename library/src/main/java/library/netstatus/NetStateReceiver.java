...
public class NetStateReceiver extends BroadcastReceiver {
    ...
    private static library.netstatus.NetUtils.NetType mNetType;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION) || intent.getAction().equalsIgnoreCase(CUSTOM_ANDROID_NET_CHANGE_ACTION)) {
            if (!library.netstatus.NetUtils.isNetworkAvailable(context)) {
                TLog.i(TAG, "<--- network disconnected --->");
                isNetAvailable = false;
            } else {
                TLog.i(TAG, "<--- network connected --->");
                isNetAvailable = true;
                mNetType = library.netstatus.NetUtils.getAPNType(context);
            }
            notifyObserver();
        }
    }
    ...
    public static library.netstatus.NetUtils.NetType getAPNType() {
        return mNetType;
    }
